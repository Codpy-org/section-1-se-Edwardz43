...
class CookieManager:
    @staticmethod
    def create_cookie(name: str, value: str, days: int = 1) -> dict:
...
async def login(request: Request, response: Response):
    ...
    return JSONResponse(
        {"message": "Login successful", "cookies": [watermelon_cookie],
         "status": 1}
    )
...
async def logout(response: Response):
    ...
    return JSONResponse(
        {"message": "Logout successful", "cookies": [], "status": 1}
    )
